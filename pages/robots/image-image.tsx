...
export default function ImageImagePage() {
  ...
  const generateImage = async (formData: FormData) => {
    const r = await fetch("/api/generate-image-image", {
      method: "POST",
      body: formData,
    });
    const d = await r.json();
  ...
  const process = async (useEdit = false) => {
    ...
    try {
      ...
      const translated = await translatePrompt(useEdit ? editPrompt : prompt);
  ...
  return (
    <>
      <Head>
        <title>Imagine + Prompt ➜ Imagine AI | Generare, Editare și Istoric | Randări 3D</title>
        <meta name="description" content="Generează imagini AI din imagine + prompt. Transformă spațiul cu stiluri moderne, lumini, texturi și ferestre. Editare, istoric și descărcare rapidă." />
        <meta property="og:title" content="Imagine + Prompt ➜ Imagine AI | Generare, Editare și Istoric | Randări 3D" />
        <meta property="og:description" content="Generează imagini AI din imagine + prompt. Transformă spațiul cu stiluri moderne, lumini, texturi și ferestre. Editare, istoric și descărcare rapidă." />
        <meta property="og:type" content="website" />
        <meta property="og:url" content="https://randari3d.ro/robots/image-image" />
        <meta property="og:image" content="https://randari3d.ro/og-image-image.jpg" />
        <meta name="twitter:title" content="Imagine + Prompt ➜ Imagine AI | Generare, Editare și Istoric | Randări 3D" />
        <meta name="twitter:description" content="Generează imagini AI din imagine + prompt. Transformă spațiul cu stiluri moderne, lumini, texturi și ferestre. Editare, istoric și descărcare rapidă." />
        <meta name="twitter:image" content="https://randari3d.ro/og-image-image.jpg" />
        <meta name="robots" content="index, follow" />
        <link rel="canonical" href="https://randari3d.ro/robots/image-image" />
      </Head>
      <main className="min-h-screen py-10 px-2 sm:px-6 md:px-12 bg-transparent text-slate-900 dark:text-white font-sans transition-colors">
        ...
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-8">
          ...
          <div className={`${glass} ${border} ${rounded} p-8 shadow-2xl flex flex-col gap-4`}>
            ...
            <div>
              <div className="flex items-center justify-between">
                <label htmlFor="prompt" className="text-xs text-slate-500">Prompt</label>
            ...
            <div className="grid grid-cols-2 gap-2 mb-1">
              {PROMPT_PRESETS.map((p, i) => (
                <button
                  ...
                  aria-label={`Selectează preset: ${p}`}
                >
            ...
            <button
              ...
              disabled={loading}
              className={`w-full flex justify-center items-center gap-2 py-3 text-base font-semibold ${gradientBtn} ${rounded} hover:scale-105 transition focus:ring-2 focus:ring-blue-400 dark:focus:ring-purple-700 outline-none ${loading ? "opacity-70 cursor-not-allowed" : ""}`}
              aria-label="Generează imagine"
            >
              {loading ? (
          ...
          <div className={`${glass} ${border} ${rounded} p-8 shadow-2xl flex flex-col gap-5 relative`}>
            ...
            {/* Buton descarcă (deschide într-un tab nou) */}
            {result && (
              <button
              ...
              <ReactCompareSlider
                itemOne={
                  <ReactCompareSliderImage
                    src={originalPreview || "/Imagine+Prompt-Imagine-inainte.jpg"}
                ...
                itemTwo={
                  <ReactCompareSliderImage
                    src={result || "/Imagine+Prompt-Imagine-dupa.jpg"}
            ...
            {result && (
              <>
                <label htmlFor="editPrompt" className="sr-only">Prompt pentru editare imagine</label>
                <input
                  id="editPrompt"
                  type="text"
                  value={editPrompt}
                  ...
                  aria-label="Prompt pentru editarea imaginii"
                />
                <button
                  onClick={() => process(true)}
                  disabled={loading}
                  className={`w-full flex justify-center items-center gap-2 py-3 text-base font-semibold bg-gradient-to-r from-green-600 via-emerald-600 to-emerald-700 hover:scale-105 text-white ${rounded} shadow-md transition focus:ring-2 focus:ring-green-400 dark:focus:ring-green-700 outline-none ${loading ? "opacity-70 cursor-not-allowed" : ""}`}
              ...
              {history.map((item, idx) => (
                ...
                >
                  {item.image && (
                    <img
                      src={item.image}
                      alt={`Imagine generată ${idx + 1}: ${item.prompt}`}
                  ...
                  <div className="absolute flex gap-1 left-2 right-2 -bottom-3 z-20 opacity-0 group-hover:opacity-100 transition-all">
                    ...
                    <button
                      onClick={() => window.open(item.image || undefined, "_blank", "noopener,noreferrer")}
        ...
        {modalImage && (
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-label="Previzualizare imagine generată"
          >
            <div className="relative max-w-3xl w-full p-5">
              ...
              <img src={modalImage} className="max-h-[70vh] max-w-full mx-auto rounded-xl shadow-2xl border-4 border-white dark:border-[#23263a]" alt="Imagine generată AI previzualizare" loading="lazy" />
            </div>
          </div>
        )}
      </main>
