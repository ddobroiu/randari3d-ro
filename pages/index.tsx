...
// Componentă simplă pentru efectul Before/After
function AutoImageSwitcher({ before, after }: { before: string; after: string }) {
  ...
  return (
    <div className="w-full h-48 overflow-hidden rounded-xl border border-slate-200 dark:border-slate-700 relative">
      <img
  ...
  const studios = [
    {
      ...
      // Folosim un video demonstrativ sau o imagine statică dacă nu ai video
      preview: (
        <div className="w-full h-48 bg-black rounded-xl overflow-hidden relative border border-slate-200 dark:border-slate-700 flex items-center justify-center group">
           {/* Înlocuiește cu un video real local dacă ai, ex: /demo-video.mp4 */}
  ...
  useEffect(() => {
    if (session?.user?.email) {
      fetch("/api/history-latest")
        .then((res) => res.json())
        .then((data) => setHistory(data.history || [])); // Adaptăm la structura nouă a API-ului
    }
  }, [session]);
              ...
              <div key={item.id} className="bg-white dark:bg-[#151a23] rounded-xl overflow-hidden shadow-lg border border-slate-200 dark:border-[#23263a] group">
                <div className="h-40 overflow-hidden relative">
                    {(item.imageUrl.startsWith("data:video") || item.robot.includes("video") || item.imageUrl.endsWith(".mp4")) ? (
                        ...
                        <img src={item.imageUrl} alt="History" className="w-full h-full object-cover group-hover:scale-110 transition duration-500" />
                    )}
                </div>
                <div className="p-3">
